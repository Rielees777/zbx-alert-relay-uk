...
def _email_status_line(provider_email_sent: bool | None) -> str | None:
    """Строка для сообщения мониторингу — только о НЕУДАЧЕ отправки письма
    оператору (провалилась, хотя предполагалась). Успешную отправку эта
    строка больше не подтверждает: об этом теперь говорится в первом
    предложении шаблона ("Направлено обращение оператору связи..."),
    повторять то же самое в конце незачем — дублирование текста.
    None (письмо не предполагалось, напр. перегрузка) и True (успешно
    отправлено) — в обоих случаях строка не добавляется."""
    if provider_email_sent is False:
