...
Режимы Pyrus:
  • РЕАЛЬНЫЙ (по умолчанию) — нужны PYRUS_LOGIN / PYRUS_TOKEN / PYRUS_FORM_ID;
    скрипт идёт на https://pyrus.sovcombank.ru за реальным реестром.
    Без этих переменных скрипт НЕ подменяет данные молча, а завершается с
    ошибкой и подсказкой, что задать.
...
Запуск (реальный Pyrus):
    PYRUS_LOGIN=... PYRUS_TOKEN=... PYRUS_FORM_ID=... python tests/scenario_rpm.py
...
def main() -> int:
    ...
    have_creds = all(os.environ.get(k) for k in ("PYRUS_LOGIN", "PYRUS_TOKEN", "PYRUS_FORM_ID"))
    ...
    if offline:
        ...
        print("\n[ОШИБКА] Не заданы учётные данные Pyrus — реальные данные получить нельзя.")
        print("         Задайте переменные окружения и запустите снова:")
        print("           PYRUS_LOGIN=...  PYRUS_TOKEN=...  PYRUS_FORM_ID=...")
        print("         Либо запустите на синтетическом реестре: SCENARIO_OFFLINE=1")
        return 2
