...
Что проверяет за один прогон:
  ...
  3. Pyrus   — чтение реестра каналов связи из PostgreSQL (наполняется
     проектом registry-pyrus-tasks), сопоставление задачи по IP роутера
     и извлечение договора нужной услуги (l2vpn).

Режимы Pyrus:
  • РЕАЛЬНЫЙ (по умолчанию) — параметры подключения берутся из Settings
    (config.py / .env), как и в проде (scheduler._build_matcher); скрипт
    читает реестр из PostgreSQL. Если БД не сконфигурирована — завершается
    с ошибкой, а не подменяет данные синтетикой.
  • OFFLINE (только по флагу SCENARIO_OFFLINE=1) — встроенный синтетический
    реестр, чтобы проверить механику без доступа к БД.

Креды (в .env, читает Settings):
    ZABBIX_URL, ZABBIX_TOKEN            — обязательны для Settings
    DB_HOST, DB_PORT, DB_NAME, DB_USER, DB_PASSWORD

Запуск (реальный Pyrus, из БД):
...
def _pyrus_matcher_real(settings):
    ...
    читаем из PostgreSQL (DB_*), которую наполняет registry-pyrus-tasks.
        ...
        PyrusSite(
            ...
            channels=[
                ChannelInfo(provider="Ростелеком", channel_id="RT-L2VPN-1", bandwidth=200000,
                            ...
                            service="L2VPN", email="support@rt.example"),
            ],
        ),
    ]
    return RegistryMatcher(sites), sites, []
...
def _diagnose_not_found(device_ip: str, hostname: str, sites, tasks) -> None:
    ...
    if hits:
        ...
        print("    → если field_id ≠ 9 — парсер registry-pyrus-tasks/pyrus/parser.py берёт IP не из того поля (см. _text_val fid=9).")
    else:
...
def _dump_site(site) -> None:
    ...
    for ch in site.channels:
        print(f"          • провайдер={ch.provider} | услуга={ch.service} | "
              f"email={ch.email} | договор={ch.contract} | {ch.bandwidth} Кбит/с")
...
def main() -> int:
    ...
    if offline:
        ...
        try:
            ...
            print(f"         {exc}")
            print("         Заполните .env (ZABBIX_*, DB_*) либо запустите с SCENARIO_OFFLINE=1.")
            return 2
        print(f"\n[Pyrus] БД реестра: {settings.db_host}:{settings.db_port}/{settings.db_name}")
