...
# Подстрока ключа item'а трафика ("net.if" — типовой ключ для net.if.in/out).
# Если не уверены, что это верный ключ для вашего Zabbix, — поставьте None:
# тогда скрипт выведет СПИСОК ВСЕХ item'ов хоста (только key_/name/units, без
# истории), чтобы найти реальные ключи и подставить сюда нужную подстроку.
TEST_ITEM_KEY_FILTER: str | None = "net.if"
# ───────────────────────────────────────────────────────────────────────────
