...
def _extract_cod_node(description: str | None) -> str | None:
    """Первый сегмент описания L2VPN-интерфейса ('n11-mts-l2vpn' → 'n11') —
    ...
    Только для L2VPN: у Интернет-каналов ЦОД в описании не указывается
    (белые IP, транспорт не привязан к конкретному ЦОДу) — описание там
    вида "<провайдер>-inet" (напр. "geotel-inet"), первый сегмент —
    провайдер, а не ЦОД. Функция это не проверяет сама (сегодня channel_hint
    для site-алертов всегда приходит из L2VPN-диагностики — Интернет-линки
    не пингуются), но если это изменится, вызывающий код (_disambiguate_by_cod)
    останется корректен: get_cod_by_name(<слаг провайдера>) не найдёт такой
    ЦОД и просто не разрешит неоднозначность, а не ошибётся."""
    if not description:
        return None
