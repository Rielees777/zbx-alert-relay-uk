...
# ── ВЕРХНЯЯ ГРАНИЦА ВОЗРАСТА АЛЕРТА + ПОВТОРНАЯ ОТПРАВКА (ПОКА НЕ ПОДКЛЮЧЕНО) ──
# Реализация — SentRegistry.should_process() (scheduler.py) и параметр
# process_decider у pipeline.run()/_collect_problems(). В боевом пути НЕ
# задействованы: обе константы вступят в силу только после подключения
# (инструкция — в scheduler.check_rpm). До этого поведение прежнее: верхней
# границы возраста нет, повторная отправка происходит неявно по TTL реестра.

# Верхняя граница: если ОТКРЫТЫЙ алерт старше этого времени и по нему ещё НЕ
# отправлялось уведомление — не реагируем вовсе (стартовое подавление
# «залежавшихся» проблем, напр. накопившихся за долгий простой процесса).
# В отличие от MIN_ALERT_AGE_SEC (нижняя граница) — здесь верхняя.
# None = без верхней границы.
MAX_ALERT_AGE_SEC: int | None = 24 * 3600        # 1 сутки

# Повторная отправка: если инцидент уже отработан (уведомление ушло), но и
# спустя это время всё ещё НЕ закрыт в Zabbix — уведомить повторно
# (напоминание о зависшей проблеме). На верхнюю границу возраста НЕ смотрит:
# касается уже однажды отработанных инцидентов, а не первичной реакции.
RESEND_AFTER_SEC: int = 7 * 24 * 3600            # 7 суток

# ── АНТИСПАМ ФЛАПАЮЩИХ КАНАЛОВ (ПОДКЛЮЧЕНО) ──────────────────────────────────
# После того как по каналу направлено обращение оператору, повторные проблемы
# того же канала в течение FLAP_WINDOW_SEC НЕ порождают новых писем/сообщений
# — вместо этого считаются; когда число пришедших алертов по каналу за окно
# превысит FLAP_ALERT_THRESHOLD, в чат мониторинга уходит одно сообщение о
# нестабильном (флапающем) канале. Ключ окна — (host_tech, channel_spec).
FLAP_WINDOW_SEC: int = 24 * 3600                 # сутки
FLAP_ALERT_THRESHOLD: int = 5                    # «больше 5» → сообщение о флапе
...
# Пока обрабатываются ТОЛЬКО эти BGP-группы — остальные (напр. DC-MOSCOW)
# полностью игнорируются на уровне парсинга: их каналы дают шум и ложные
# срабатывания, диагностика по ним пока не готова. Пусто — без ограничения
# (обрабатываются все группы).
PARSED_BGP_GROUPS: tuple[str, ...] = ("ebgp",)
